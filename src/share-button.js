...
function getTextContent(socialMedia) {
  switch (socialMedia) {
    case "facebook":
      return "Facebook";
    case "google-plus":
...
function getBackgroundColor(socialMedia) {
  switch (socialMedia) {
    case "facebook":
      return "#3B5998";
    case "google-plus":
...
function Wrapper({ children, socialMedia, url, text, media }) {
  const background = getBackgroundColor(socialMedia);
...
export default function SocialShareButton({
  ...
  return (
    <Wrapper socialMedia={socialMedia} url={url} text={text} media={media}>
      ...
      {info && <Text socialMedia={socialMedia} />}
    </Wrapper>
  );
}
